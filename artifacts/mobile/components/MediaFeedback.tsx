/**
 * MediaFeedback.tsx — Vedra v0.7
 *
 * Feedback card for media control commands:
 * play, pause, next, previous, stop, media volume up/down.
 */
...
export type MediaFeedbackState =
  | { phase: 'none' }
...
function StatusContent({ state }: { state: MediaFeedbackState }) {
  const colors = useColors();
  if (state.phase === 'none') return null;

  if (state.phase === 'working') {
    return (
      <Row icon="music" label="STATUS" value={
        <View style={styles.statusRow}>
          <ActivityIndicator size="small" color={colors.processingRing} />
          <Text style={[styles.rowValue, { color: colors.processingRing }]}>Sending…</Text>
        </View>
      } />
    );
  }
  ...
  if (state.phase === 'settings') {
    return (
      <>
        <Row icon="external-link" label="ACTION" value={state.detail} />
        <Divider />
    ...
    <>
      ...
      <Row icon="alert-circle" label="STATUS" value={
        <View style={styles.statusRow}>
          ...
          <Text style={[styles.rowValue, { color: colors.destructive }]}>{state.detail}</Text>
        </View>
      } />
    </>
  );
...
export default function MediaFeedback({ state }: { state: MediaFeedbackState }) {
  const colors = useColors();
  ...
  return (
    <Animated.View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }, animStyle]}>
      ...
      <Divider />
      <Row icon="headphones"     label="COMMAND"   value={commandLabel} />
      <Divider />
